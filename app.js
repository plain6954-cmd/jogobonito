...
const pmCardContainer = document.getElementById('pm-card-container');
const pmEditBtn = document.getElementById('pm-edit-btn');
let currentlyViewedPlayerId = null;

pmCloseBtn.addEventListener('click', () => {
    playerModal.classList.remove('show');
    pmCardContainer.innerHTML = '';
});
...
pmEditBtn.addEventListener('click', () => {
    playerModal.classList.remove('show');
    pmCardContainer.innerHTML = '';
    if (state.isAdmin) editPlayer(currentlyViewedPlayerId);
});
...
function renderPlayers(filter = 'ALL') {
    ...
    filteredPlayers.forEach(player => {
        ...
        card.addEventListener('click', () => {
            showPlayerModal(player, card.innerHTML);
        });
...
function showPlayerModal(player, cardHtml) {
    currentlyViewedPlayerId = player.id;
    
    const zoomedCard = document.createElement('div');
    zoomedCard.className = 'player-card zoomed';
    zoomedCard.innerHTML = cardHtml;
    
    zoomedCard.style.pointerEvents = 'none';
    zoomedCard.style.transform = 'scale(1.3)';
    zoomedCard.style.margin = '40px';
    
    pmCardContainer.innerHTML = '';
    pmCardContainer.appendChild(zoomedCard);
    
    if (state.isAdmin) {
        pmEditBtn.classList.remove('hidden');
    } else {
        pmEditBtn.classList.add('hidden');
    }
